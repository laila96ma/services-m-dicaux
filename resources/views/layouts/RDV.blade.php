                        ...
                        <div class="text-green-500 text-lg mb-4 flex justify-between mt-7">
                            <div>
                               ...
                               <div>
                                <a href="{{ url('/RDV/create') }}" class="btn btn-success btn-sm" title="Add New rendez-vous">
                                    <i class="fa fa-plus" aria-hidden="true"></i> ajouter un rendez-vous
                                </a>
                               </div>
                            ...
                            <div class="card-body flex justify-between items-center">
                                <div class="overflow-x-auto">
                                    @if ($RDV->isEmpty())
                                    <p class="text-lg text-red-700 ml-96">Aucun rendez-vous trouvé.</p>
                                @else
                            <table class="table-auto w-full">
                                ...
                                @foreach($RDV as $item)
                                    <tr>
                                        ...
                                        @if ($item->patient)
                                            <td class="border px-1 py-2">{{ $item->patient->matricule }}</td>
                                            <td class="border px-1 py-2">{{ $item->patient->nom }}</td>
                                            <td class="border px-1 py-2">{{ $item->patient->poste }}</td>
                                            <td class="border px-1 py-2">{{ $item->patient->service }}</td>
                                            <td class="border px-4 py-2">
                                                ...
                                                <form method="POST" action="{{ url('/RDV' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                    ...
                                                    <a href="{{ url('/download/'.$item->id) }}" class="btn btn-primary btn-sm" title="Télécharger">
                                    ...
                                    @else
                                        <td colspan="3">Aucune patient associée à ce rendez-vous.</td>
                                    @endif
